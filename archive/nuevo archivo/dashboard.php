...
/* ================= Cálculo de KPIs por rango ================= */
/* AGUA (Cruda+Tratada) y SUPERVISIÓN — usando SIEMPRE COALESCE(v.VENTFEVE, f.FACTFECH) */
if ($HAS_DET){
  $sqlK = "
    SELECT
      SUM(
...
/* ===== DONA: PENDIENTES por categoría (AC / AT / SV) — YA RESPETA EL RANGO ===== */
if ($HAS_DET){
  $sqlDonut = "
    SELECT
      SUM(
...
.btn-volver{ justify-self:start; display:inline-flex; align-items:center; gap:8px; background:#fff; color:#0b3850; border:none; border-radius:9999px; padding:8px 14px; font-weight:600; text-decoration:none; box-shadow:0 2px 6px rgba(0,0,0,.08); }
.topbar-title{ margin:0; color:#fff; font-size:22px; font-weight:800; text-align:center; }

/* User pill + dropdown (solo Cerrar sesión) */
...
<header class="topbar">
  <div class="topbar-inner">
    ...
    <h1 class="topbar-title">Panel de Control — EMDUPAR</h1>
    ...
    <div class="user-box">
      ...
      <div id="userMenu" class="user-menu" aria-hidden="true">
        <!-- Solo cerrar sesión (en rojo) -->
        <a class="user-danger" href="logout.php"><i class="bi bi-box-arrow-right"></i> Cerrar sesión</a>
      </div>
    </div>
  </div>
</header>
...
<div class="container-xxl my-4">
  ...
  <div class="row g-3">
    <div class="col-12 col-md-6 col-lg-3">
      <div class="card p-3 kpi">
        ...
        <small class="text-muted">Agua Cruda + Tratada · <?php echo htmlspecialchars($desde); ?> — <?php echo htmlspecialchars($hasta); ?></small>
      </div>
    </div>
    <div class="col-12 col-md-6 col-lg-3">
      <div class="card p-3 kpi">
        <div class="label">Ventas SUPERVISIÓN (rango)</div>
        <div class="value"><?php echo cop($ventasRangoSup); ?></div>
        <small class="text-muted">Instalación de medidores · <?php echo htmlspecialchars($desde); ?> — <?php echo htmlspecialchars($hasta); ?></small>
      </div>
    </div>
    <div class="col-12 col-md-6 col-lg-3">
      <div class="card p-3 kpi">
        <div class="label">Facturas del período</div>
        <div class="value"><a href="<?php echo htmlspecialchars($linkReportes); ?>"><?php echo $facturasPeriodo; ?></a></div>
        <small class="text-muted">Abrir en Reportes · Ventas</small>
      </div>
    </div>
    <div class="col-12 col-md-6 col-lg-3">
      <div class="card p-3 kpi">
        ...
        <small class="text-muted"><?php echo htmlspecialchars($desde); ?> — <?php echo htmlspecialchars($hasta); ?></small>
      </div>
    </div>
    <div class="col-12 col-md-6 col-lg-3">
      <div class="card p-3 kpi">
  ...
  <div class="row g-3 mt-1">
    <div class="col-lg-8">
      <div class="card p-3">
        <h5 class="mb-3">Ventas por mes (según rango)</h5>
        <canvas id="cCombinado"></canvas>
      </div>
    </div>
    <div class="col-lg-4">
      <div class="card p-3">
        <h5 class="mb-3">Pendientes por categoría</h5>
  ...
  <div class="row g-3 mt-1">
    <div class="col-lg-4">
      <div class="card p-3">
        <h5 class="mb-3">Agua Cruda — en rango</h5>
        <canvas id="cAguaCruda"></canvas>
      </div>
    </div>
    <div class="col-lg-4">
      <div class="card p-3">
        <h5 class="mb-3">Agua Tratada — en rango</h5>
        <canvas id="cAguaTratada"></canvas>
      </div>
    </div>
    <div class="col-lg-4">
      <div class="card p-3">
        <h5 class="mb-3">Supervisión — en rango</h5>
...
var SV6 = <?php echo json_encode($sv6); ?>;   // Supervisión
...
new Chart(document.getElementById('cCombinado').getContext('2d'), {
  ...
  data:{
    ...
    datasets:[
      ...
      { label:'Supervisión',  data:SV6, borderWidth:1 }
    ]
  },
  options:{
    responsive:true,
...
/* Dona de pendientes por categoría (en rango) */
var ctxD=document.getElementById('cDonaPend').getContext('2d');
new Chart(ctxD,{
  type:'doughnut',
  data:{ labels:['Pend. Agua Cruda','Pend. Agua Tratada','Pend. Supervisión'], datasets:[{ data:PEND, borderWidth:1 }] },
  options:{ plugins:{ legend:{ position:'bottom' } }, cutout:'55%' }
});

/* Gráficas individuales (en rango) */
...
simpleBar('cSupervision','Supervisión',SV6);
</script>
</body>
</html>
