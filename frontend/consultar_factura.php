...
function factura_pdf_exists($id){
  $path = dirname(__DIR__) . '/facturas_pdf/factura_'.$id.'.pdf';
  return file_exists($path);
}

/* ================= 1) Modo: abrir por n=? (GET) ================= */
if (isset($_GET['n']) && ctype_digit($_GET['n'])) {
  $_POST['FACTRECI'] = $_GET['n'];
...
if (isset($_GET['estado'])) {
  ...
  if (in_array($estado_filtro, array('PAGADA','PENDIENTE','ENTREGADA'), true)) {
    ...
    try {
      $stC = $pdo->prepare("SELECT COUNT(*) FROM facturas WHERE UPPER(estado)=?");
      $stC->execute(array($estado_filtro));
      $total_reg = (int)$stC->fetchColumn();

      $sql = "
        SELECT f.id AS fact, f.fecha AS fecha, f.cliente_id AS doc,
               COALESCE(c.nombre, CAST(f.cliente_id AS CHAR)) AS nomb,
               f.total AS valor, f.estado AS est, f.fecha_entrega AS entr, f.fecha_pago AS fepa
        FROM facturas f
        LEFT JOIN clientes c ON c.id = f.cliente_id
        WHERE UPPER(f.estado) = ?
  ...
  if ($numero !== '' && ctype_digit($numero)) {
    ...
    $sql = "
      SELECT 
          ...
          f.total AS valor,
          f.estado AS est,
          f.fecha_entrega AS entr,
          f.fecha_pago AS fepa,
          f.placa AS placa,
          v.cantidad AS cant,
          p.PRODDESC AS prod
      FROM facturas f
...
<main>
  <div class="card">
    ...
    <?php if (!$modo_lista && $resultado): ?>
      ...
      <div class="resultado">
        <div class="resultado__header">
          Factura N° <?php echo htmlspecialchars($resultado['fact']); ?> &nbsp;<?php echo badge_html($estadoN); ?>
        </div>
    ...
    <?php if ($modo_lista): ?>
      ...
      <?php else: ?>
        <div class="alert alert-ok">Total: <?php echo (int)$total_reg; ?> · Página <?php echo (int)$pag; ?></div>
