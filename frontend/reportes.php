<?php
/* reportes.php Ã¢â‚¬â€ Ventas/Entregas (con columnas Producto y Cantidad)
   - Muestra y exporta Producto.
   - Columna "Cantidad" tomada de ventas.VENTCANT.
   - Exporta CSV y PDF (FPDF).
   - Permisos: rep_ventas / rep_entregas (lectura).
   - Header con tÃƒÂ­tulo centrado + pill de usuario y menÃƒÂº Ã¢â‚¬Å“Cerrar sesiÃƒÂ³nÃ¢â‚¬Â.
*/
...
/* Normaliza fechas de filtros: acepta dd/mm/yyyy o yyyy-mm-dd y devuelve yyyy-mm-dd */
function normalizeDateInput($str){
  ...
  // yyyy-mm-dd ya vÃƒÂ¡lido
  if (preg_match('/^\d{4}-\d{2}-\d{2}$/', $str)) {
    return $str;
  }
  // ÃƒÅ¡ltimo recurso: strtotime
  $t = strtotime($str);
  return $t ? date('Y-m-d', $t) : '';
}

/* Formatea una fecha yyyy-mm-dd a dd/mm/yyyy para textos de subtÃƒÂ­tulos */
function fmtSubDate($ymd){
  $ymd = trim((string)$ymd);
  if ($ymd === '') return '';
  $t = strtotime($ymd);
...
/* === Logo robusto para FPDF (acepta PNG/JPG y Ã¢â‚¬Å“aplanaÃ¢â‚¬Â si el PNG estÃƒÂ¡ raro) === */
...
/* Fechas normalizadas para usar en SQL (yyyy-mm-dd) */
$pv_desde_ventas  = isset($_POST['desde_ventas'])   ? normalizeDateInput($_POST['desde_ventas'])   : '';
$pv_hasta_ventas  = isset($_POST['hasta_ventas'])   ? normalizeDateInput($_POST['hasta_ventas'])   : '';
...
$pv_desde_entregas   = isset($_POST['desde_entregas'])   ? normalizeDateInput($_POST['desde_entregas'])   : '';
$pv_hasta_entregas   = isset($_POST['hasta_entregas'])   ? normalizeDateInput($_POST['hasta_entregas'])   : '';
...
/* ===== DefiniciÃƒÂ³n de "Producto" y "Cantidad" ===== */
...
if ($HAS_DET) {
  ...
  $prodExpr    = "COALESCE(pr.PRODUCTO, $baseProdDesc, 'Ã¢â‚¬â€')";
} else {
  $prodJoin    = "";
  $prodExpr    = "COALESCE($baseProdDesc, 'Ã¢â‚¬â€')";
}

/* Cantidad SIEMPRE desde ventas.VENTCANT */
$qtyExpr = "COALESCE(v.VENTCANT, 0)";
...
if (isset($_POST['do_export']) && in_array((isset($_POST['which']) ? $_POST['which'] : ''), array('ventas','entregas'), true)) {
  ...
  if ($which === 'ventas') {
    ...
    $desde = isset($_POST['desde_ventas']) ? normalizeDateInput($_POST['desde_ventas']) : '';
    $hasta = isset($_POST['hasta_ventas']) ? normalizeDateInput($_POST['hasta_ventas']) : '';
    ...
    $subtitle = 'Desde '.fmtSubDate(isset($params[0]) ? $params[0] : '').
                ' hasta '.fmtSubDate(isset($params[1]) ? $params[1] : '').
                (isset($params[2]) ? (' Ã¢â‚¬Â¢ Usuario: '.(isset($_POST['usuario_ventas']) ? $_POST['usuario_ventas'] : '')):'');
    ...
    $desde = isset($_POST['desde_entregas']) ? normalizeDateInput($_POST['desde_entregas']) : '';
    $hasta = isset($_POST['hasta_entregas']) ? normalizeDateInput($_POST['hasta_entregas']) : '';
    ...
    $subtitle = 'Desde '.fmtSubDate(isset($params[0]) ? $params[0] : '').
                ' hasta '.fmtSubDate(isset($params[1]) ? $params[1] : '').
                (isset($params[2]) ? (' Ã¢â‚¬Â¢ Usuario: '.(isset($_POST['usuario_entregas']) ? $_POST['usuario_entregas'] : '')):'');
  }
  ...
  if ($format === 'excel') {
    ...
    if ($which==='ventas' && in_array('Valor',$headers,true)) {
      ...
      fputcsv($out, array('', 'TOTAL SUPERVISIÃƒâ€œN',            '', '', '', $totalSup),   ';');
    }
    ...
    class PrettyPDF extends FPDF {
      ...
      function Footer(){
        ...
        $this->Cell(0,10,enc('Generado: '.date('d/m/Y H:i').' Ã¢â‚¬Â¢ PÃƒÂ¡gina '.$this->PageNo().'/{nb}'),0,0,'R');
      }
    ...
    if ($which==='ventas' && isset($totalAguas) && isset($totalSup)) {
      ...
      $extraTotals[] = array('label'=>'TOTAL SUPERVISIÃƒâ€œN',            'value'=>$totalSup);
    }
...
<head>
  <meta charset="UTF-8" />
  <title>MÃƒÂ³dulo de Reportes</title>
  ...
  header,.topbar{margin-top:0 !important;}
    ...
    /* Pill de usuario + dropdown (no rompe diseÃƒÂ±o) */
    .user-pill{
      margin-left:auto;
      display:flex;
      align-items:center;
...
<body>
  ...
  <!-- Header: botÃƒÂ³n Volver (izq), tÃƒÂ­tulo centrado absoluto, usuario (der) -->
  <header class="d-flex align-items-center position-relative">
    <a href="menu_principal.php" class="btn btn-outline-light">
    ...
    <div class="user-menu" id="userMenu" aria-hidden="true">
      <a href="#" id="openLogout" class="user-item">
        <i class="bi bi-box-arrow-right"></i> Cerrar sesiÃƒÂ³n
      </a>
    </div>
  </header>
                    ...
                    <thead>
                      <tr>
                        <th onclick="sortBy('ventas','factura')">
                          Factura
                          <?php echo ($pv_sortf_ventas==='factura' ? '<span class="arrow">'.($pv_sortd_ventas==='asc'?'Ã¢â€ â€˜':'Ã¢â€ â€œ').'</span>' : ''); ?>
                        </th>
                        <th onclick="sortBy('ventas','producto')">
                          Producto
                          <?php echo ($pv_sortf_ventas==='producto' ? '<span class="arrow">'.($pv_sortd_ventas==='asc'?'Ã¢â€ â€˜':'Ã¢â€ â€œ').'</span>' : ''); ?>
                        </th>
                        <th onclick="sortBy('ventas','cantidad')">
                          Cantidad
                          <?php echo ($pv_sortf_ventas==='cantidad' ? '<span class="arrow">'.($pv_sortd_ventas==='asc'?'Ã¢â€ â€˜':'Ã¢â€ â€œ').'</span>' : ''); ?>
                        </th>
                        <th onclick="sortBy('ventas','fecha')">
                          Fecha
                          <?php echo ($pv_sortf_ventas==='fecha' ? '<span class="arrow">'.($pv_sortd_ventas==='asc'?'Ã¢â€ â€˜':'Ã¢â€ â€œ').'</span>' : ''); ?>
                        </th>
                        <th onclick="sortBy('ventas','usuario')">
                          Usuario
                          <?php echo ($pv_sortf_ventas==='usuario' ? '<span class="arrow">'.($pv_sortd_ventas==='asc'?'Ã¢â€ â€˜':'Ã¢â€ â€œ').'</span>' : ''); ?>
                        </th>
                        <th class="text-end" onclick="sortBy('ventas','valor')">
                          Valor
                          <?php echo ($pv_sortf_ventas==='valor' ? '<span class="arrow">'.($pv_sortd_ventas==='asc'?'Ã¢â€ â€˜':'Ã¢â€ â€œ').'</span>' : ''); ?>
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      <?php for ($i=0; $i<count($ventas_rows); $i++): $r=$ventas_rows[$i]; ?>
                        <tr>
                          ...
                          <td><?php echo htmlspecialchars(isset($r['Producto']) ? $r['Producto'] : 'Ã¢â‚¬â€'); ?></td>
                    ...
                    <thead>
                      <tr>
                        <th onclick="sortBy('entregas','factura')">
                          Factura
                          <?php echo ($pv_sortf_entregas==='factura' ? '<span class="arrow">'.($pv_sortd_entregas==='asc'?'Ã¢â€ â€˜':'Ã¢â€ â€œ').'</span>' : ''); ?>
                        </th>
                        <th onclick="sortBy('entregas','producto')">
                          Producto
                          <?php echo ($pv_sortf_entregas==='producto' ? '<span class="arrow">'.($pv_sortd_entregas==='asc'?'Ã¢â€ â€˜':'Ã¢â€ â€œ').'</span>' : ''); ?>
                        </th>
                        <th onclick="sortBy('entregas','cantidad')">
                          Cantidad
                          <?php echo ($pv_sortf_entregas==='cantidad' ? '<span class="arrow">'.($pv_sortd_entregas==='asc'?'Ã¢â€ â€˜':'Ã¢â€ â€œ').'</span>' : ''); ?>
                        </th>
                        <th onclick="sortBy('entregas','fecha')">
                          Fecha Entrega
                          <?php echo ($pv_sortf_entregas==='fecha' ? '<span class="arrow">'.($pv_sortd_entregas==='asc'?'Ã¢â€ â€˜':'Ã¢â€ â€œ').'</span>' : ''); ?>
                        </th>
                        <th onclick="sortBy('entregas','usuario')">
                          Usuario
                          <?php echo ($pv_sortf_entregas==='usuario' ? '<span class="arrow">'.($pv_sortd_entregas==='asc'?'Ã¢â€ â€˜':'Ã¢â€ â€œ').'</span>' : ''); ?>
                        </th>
                      </tr>
                    </thead>
                    <tbody>
    ...
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <p class="mb-3">Ã‚Â¿En quÃƒÂ© formato deseas descargarlo?</p>
  ...
  <!-- Modal de Cerrar sesiÃƒÂ³n -->
  <div class="modal fade" id="logoutModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header" style="background:#1f6691;color:#fff;">
          <h5 class="modal-title"><i class="bi bi-box-arrow-right"></i> Cerrar sesiÃƒÂ³n</h5>
          <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Cerrar"></button>
        </div>
        <div class="modal-body">Ã‚Â¿Deseas salir del sistema?</div>
        <div class="modal-footer" style="background:#f7fbff;">
          <button type="button" class="btn btn-light" data-bs-dismiss="modal">Cancelar</button>
          <a class="btn btn-danger" href="logout.php">
            <i class="bi bi-door-open"></i> SÃƒÂ­, salir
          </a>
        </div>
      </div>
    </div>
  ...
  <script>
    ...
    /* ==== MenÃƒÂº de usuario + Cerrar sesiÃƒÂ³n ==== */
    (function(){
