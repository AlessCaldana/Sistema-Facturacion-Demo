  ...
  */
  ...
  try {
    if ($accion === 'insertar') {
      ...
      // Cargar datos originales para bloquear cambios de tipo y número de documento
      $stOrig = $pdo->prepare("SELECT VEHITDOC, VEHIDOCU, VEHICOND FROM vehiculos WHERE VEHIPLAC = ?");
      $stOrig->execute(array($placa_o));
      $orig = $stOrig->fetch(PDO::FETCH_ASSOC);

      if ($orig) {
        // Tipo de documento original
        $orig_tdoc = (isset($orig['VEHITDOC']) ? $orig['VEHITDOC'] : '');

        // Si ya tenía tipo de documento guardado, no permitir cambiarlo ni borrarlo
        if ($orig_tdoc !== '' && $orig_tdoc !== null && $tdoc !== $orig_tdoc) {
          $go('', '❌ No se permite modificar el tipo de documento del conductor.');
        }

        // Número de documento original (VEHIDOCU o VEHICOND)
        $orig_doc = '';
        if (isset($orig['VEHIDOCU']) && $orig['VEHIDOCU'] !== '' && $orig['VEHIDOCU'] !== null) {
          $orig_doc = $orig['VEHIDOCU'];
        } elseif (isset($orig['VEHICOND']) && $orig['VEHICOND'] !== '' && $orig['VEHICOND'] !== null) {
          $orig_doc = $orig['VEHICOND'];
        }

        // Si ya tenía número documento, no permitir cambiarlo ni borrarlo
        if ($orig_doc !== '' && $orig_doc !== null && $ndoc !== $orig_doc) {
          $go('', '❌ No se permite modificar el número de documento del conductor.');
        }

        // Forzar a usar siempre el original en caso de que el formulario no lo envíe
        if ($orig_tdoc !== '' && $orig_tdoc !== null) {
          $tdoc = $orig_tdoc;
        }
        if ($orig_doc !== '' && $orig_doc !== null) {
          $ndoc = $orig_doc;
          $docCompat = ($tdoc === 'PA') ? null : $ndoc;
        }
      }
    ...
    <div class="card card-glass p-3 mb-4">
      ...
      <form method="POST" class="row g-3">
        ...
        <div class="col-md-3">
          ...
          <input type="text" name="VEHIMODE" class="form-control"
        ...
        <div class="col-md-4">
          ...
          <select name="VEHITDOC" id="VEHITDOC" class="form-select"
                  <?php echo $vehiculoEditar ? 'disabled' : ''; ?>>
          ...
          <?php if ($vehiculoEditar): ?>
            <!-- Campo oculto para enviar el tipo original aunque el select esté deshabilitado -->
            <input type="hidden" name="VEHITDOC" value="<?php echo htmlspecialchars($curTdoc); ?>">
          <?php endif; ?>
        </div>
